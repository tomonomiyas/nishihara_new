...
function initFormValidation() {
  // 入力画面のフォームのみ対象（確認画面は hidden だけなので除外）
  const form = document.querySelector(".p-form:not(.p-form--confirm)");
  if (!form) return;

  // 送信時のバリデーション（サーバー側でも検証するため、JSは補助。
  // 「内容を修正する」等 formnovalidate のボタンや確認画面送信は妨げない）
  form.addEventListener("submit", e => {
    const submitter = e.submitter;
    // 修正ボタン等 formnovalidate のものはそのまま送信させる
    ...
    // バリデーションOKなら preventDefault せず、PHP（確認画面）へPOST送信させる
  });
